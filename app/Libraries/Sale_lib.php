...
class Sale_lib
{
    ...
    {
        if ($this->session->get('sales_luna_id') === null) {
            $this->set_luna_id(-1);
        }

        return (int) $this->session->get('sales_luna_id');
    }

    public function set_luna_id(int $luna_id): void
    {
        $this->session->set('sales_luna_id', $luna_id);
    }

    public function remove_luna_id(): void
    {
        $this->session->remove('sales_luna_id');
    }
