...
class Sales extends Secure_Controller
{
    ...
    public function postSelectCustomer(): ResponseInterface|string
    {
        ...
        if ($this->customer->exists($customer_id)) {
            if ($customer_id !== $this->sale_lib->get_customer()) {
    ...
    /**
     * Stores the currently selected luna for the in-progress sale.
     *
     * @noinspection PhpUnused
     */
    public function postSelectLuna(): ResponseInterface|string
    {
        ...
        $this->sale_lib->set_luna_id($selected_luna ? (int) $selected_luna->luna_id : -1);
     ...
     * @return void
     */
    private function _get_sale_supplier_info(int $customer_id): ?object
    {
        if ($customer_id === NEW_ENTRY) {
            return null;
        }

        return $this->supplier->get_info_by_customer_id($customer_id);
    }

    private function _get_sale_lunas(?object $supplier_info): array
    {
        if ($supplier_info === null) {
            return [];
        }

        return match ((int) $supplier_info->category) {
            LAND_OWNER_SUPPLIER => $this->luna->get_lunas_for_landowner((int) $supplier_info->person_id),
            TENANT_SUPPLIER     => $this->luna->get_lunas_for_tenant((int) $supplier_info->person_id),
            default             => [],
        };
    }
    ...
    private function _get_valid_sale_luna(int $customer_id, int $luna_id, ?object $supplier_info = null, array $lunas = []): ?object
    {
        ...
        if (! in_array($luna_id, $valid_luna_ids, true)) {
            return null;
        }

        $selected_luna = $this->luna->get_info($luna_id);
        if ($selected_luna === null) {
            return null;
        }

        $supplier_id = (int) $supplier_info->person_id;

        return match ((int) $supplier_info->category) {
            LAND_OWNER_SUPPLIER => (int) $selected_luna->landowner_id === $supplier_id ? $selected_luna : null,
            ...
            if ($loan_amount > 0) {
                ...
                if ($selected_luna !== null) {
                    ...
                    if (! empty($selected_luna->barangay)) {
                        $luna_label .= ' (' . $selected_luna->barangay . ')';
                    }
                }

                $comment = lang('Sales.loan_recorded', [to_currency($loan_amount), $customer_name]);
                if ($luna_label !== '') {
                    $comment .= ' ' . lang('Sales.luna') . ': ' . $luna_label . '.';
                }
                ...
                $customer_loan->record_loan(
                    ...
                    null,
                    $comment,
                    $selected_luna ? (int) $selected_luna->luna_id : null,
                );
            }
        }
    }
    ...
    private function _load_customer_data(int $customer_id, array &$data, bool $stats = false): array|stdClass|null    // TODO: Hungarian notation
    {
        $customer_info                      = null;
        $data['lunas']                      = [];
            ...
            if ($supplier_info !== null) {
                ...
                if (! empty($data['lunas'])) {
                    ...
                    if ($selected_luna === null) {
                        ...
                        $data['customer_luna_loan_balance'] = $customer_loan->get_loan_balance_for_luna($customer_id, (int) $selected_luna->luna_id);
                    }
                } else {
                    $this->sale_lib->set_luna_id(-1);
                }
